...
def noise_estimation_loss(model, x0, t, e, b):
    ...
    # print("x0",x0.shape,"e",e.shape,"x",x.shape,t.shape)
    output = model(torch.cat([x0[:, :3, :, :], x], dim=1), t.float())
    # print("e",e.shape,"output",output.shape)
...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        # self.model = My_DiT_test(input_size=64)
        # self.model.cuda()
        # self.model_name = 'DiT'
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                x = torch.cat((inp,gt),dim=1)
                # print('-inp-',inp.shape,'-gt-',gt.shape,'-x-',x.shape)
                n = x.size(0)
                ...
                if self.step%1000==0:
                    ...
                    for io in range(10):
                        all_torchcat = torch.cat( ( x[io:io+1,0:3,:,:],
                                                    x[io:io+1,3:6,:,:],
                                                    x[io:io+1,6:9,:,:],
                                                    x[io:io+1,9:12,:,:]
                                                    ),dim=-1)
                        tvu.save_image(all_torchcat, os.path.join(image_folder, "train"+"_"+f"{io}_inp_gt_oup.jpg"))


                x = data_transform(x).cuda()
                e = torch.randn_like(x[:, 3:, :, :]).cuda()
                b = self.betas

                # antithetic sampling
                t = torch.randint(low=0, high=self.num_timesteps, size=(n // 2 + 1,)).cuda()
    ...
    def sample_validation_patches(self, val_loader, step):
        ...
        with torch.no_grad():
            ...
            x = torch.randn(n, 9, self.config.data.image_size, self.config.data.image_size, device=self.device)
            x = self.sample_image(x_cond, x)
            print("output",x.shape)
            x = inverse_data_transform(x)

            oups1 = inverse_data_transform(x[:,0:3,:,:].clone()).cpu()
            oups2 = inverse_data_transform(x[:,3:6,:,:].clone()).cpu()
            oups3 = inverse_data_transform(x[:,6:9,:,:].clone()).cpu()
            vis_xinp = inverse_data_transform(x_cond).clone().cpu()
